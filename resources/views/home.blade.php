    ...
    <header class="bg-cover drop-shadow rounded-b bg-gradient-to-r from-[#f75394] to-yellow-300 py-20 text-center relative">
        ...
        <div class="my-6 m-auto w-[60vh] sm:w-[80vh] relative opacity-0 duration-700"
            ...
            <form action="{{ route('event.showMyEvent', ['events'=>$events])}}" method="GET">
                @csrf
                <button type="submit"
                    class="bg-[#f3efef] p-1 px-10 rounded transition-all hover:opacity-80">My Event</button>
            </form>
    ...
    <div class=" mx-auto px-4 py-8 rounded-lg sm:px-8 md:px-12 lg:px-16 transition-all">
        <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3">
            @foreach ($events as $event)
                <a href="{{ route('events.show', ['event'=> $event])}}" class="bg-white transition-all duration-1000 hover:scale-105 rounded-lg shadow-md p-6 opacity-0"
                    data-replace='{"opacity-0":"opacity-100"}'>
