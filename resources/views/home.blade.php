...
@section('content')
    ...
    <div class=" mx-auto px-4 py-8 rounded-lg sm:px-8 md:px-12 lg:px-16 transition-all">
        <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3">
            @foreach ($events as $event)
                <a href="{{ route('events.show', ['event'=> $event])}}" class="bg-white transition-all duration-1000 hover:scale-110 rounded-lg shadow-md p-6 opacity-0"
                    data-replace='{"opacity-0":"opacity-100"}'>
    ...
    <script src="{{ asset('js/event-filter.js') }}"></script>
