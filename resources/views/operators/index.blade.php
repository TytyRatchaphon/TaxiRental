        ...
        <div class="flex justify-between mb-5">
            ...
            <div>
                <form action="{{ route('operators.search') }}" method="get">
                    @csrf
                    @method('GET')
                    <input type="text" name="input" placeholder="Search Operator">
                    <button type="submit"
                ...
                <div class="flex bg-white rounded-lg shadow-lg w-full p-3 items-center justify-between">
                    <div class="flex w-full items-center">
                        <div class="flex items-center w-3/12">
                            <img src="https://randomuser.me/api/portraits/men/32.jpg" alt="profile_img"
                                class="w-8 h-8 rounded-full mr-2 object-cover">
                            <span class="text-gray-800 font-semibold mr-5">{{ $operator->username }}</span>
                        </div>
                        <div class="">
                            <h1 class="font-semibold">{{ $operator->user_firstname }} {{ $operator->user_lastname }}
                            </h1>
                        </div>
                    </div>
                    <!-- Only Admin -->
