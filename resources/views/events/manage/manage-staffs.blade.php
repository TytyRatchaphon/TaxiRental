                    ...
                    @if (
                        ...
                        <li class="flex flex-col bg-white rounded-lg shadow-lg overflow-hidden p-7 mb-5">
                            <div class="flex items-center mb-3">
                                ...
                                <!-- Reject Button -->
                                <form action="{{ route('detach', ['event' => $event->id, 'student' => $student->id]) }}" method="POST">
                                    @csrf
                                    <button type="submit" class="bg-[#FF6666] font-semibold text-white hover:opacity-80 rounded-lg p-1 pr-2 pl-2">Delete</button>
                                </form>
                ...
                <ul class="flex flex-col w-full">
                    <div class="flex flex-col bg-white rounded-lg shadow-lg overflow-hidden p-7 mb-5">
                        <form action="{{ route('events.addStaff', ['event' => $event->id]) }}" method="POST">
                            ...
                            <input type="text" id="username" name="username" class="border p-2 rounded-lg mb-3" required>
                            <input type="submit"
