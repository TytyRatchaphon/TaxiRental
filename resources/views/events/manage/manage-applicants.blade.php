            ...
            <ul class="flex flex-col">
                <li class="flex flex-col bg-white rounded-lg shadow-lg overflow-hidden p-7 mb-5">
                    <div class="flex items-center mb-3">
                        ...
                        <button
                            class="bg-[#FF6666] font-semibold text-white hover:opacity-80 rounded-lg p-1 pr-2 pl-2">ยกเลิก</button>
                    </div>
                    <hr>
                    <div class="pt-3">
        ...
        <div class="w-1/2">
            ...
            <ul class="flex flex-col w-full">
                @foreach ($students as $student)
                    ...
                    @if ($event->students()->where('student_id', $student->id)->first()->pivot->role != 'HEAD')
                    ...
                    <li class="flex flex-col bg-white rounded-lg shadow-lg overflow-hidden p-7 mb-5">
                        <div class="flex items-center mb-3">
                            ...
                            <form action="{{ route('approve', ['event' => $event->id, 'student' => $student->id]) }}" method="POST">
                                @csrf
                                <button type="submit" class="bg-green-500 font-semibold text-white hover:opacity-80 rounded-lg p-1 pr-2 pl-2">Approve</button>
                            </form>
                            
                            <!-- Reject Button -->
                            <form action="{{ route('reject', ['event' => $event->id, 'student' => $student->id]) }}" method="POST">
                                @csrf
                                <button type="submit" class="bg-[#FF6666] font-semibold text-white hover:opacity-80 rounded-lg p-1 pr-2 pl-2">Reject</button>
                            </form>
                        ...
                        <div class="pt-3">
                            ...
                            <h1>Faculty : {{ $student->faculty }} Major : {{ $student->major }} Year : {{ $student->year }}
                            </h1>
