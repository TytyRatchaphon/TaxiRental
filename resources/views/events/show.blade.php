        ...
        <div class=" m-32 mt-10">
            <div class="flex flex-inline space-x-4">
                ...
                <ul class="flex p-2 pl-0 mb-5">
                    <li class="flex bg-[#F6D106] p-1 pl-3 pr-3 mr-5 rounded"><i
                            ...
                            style="color: black;"></i>{{ $event->students()->byRoleEvent('HEAD')->first()->user->username }}
                    </li>
                    ...
                    @if (Auth::check() && Auth::user()->byRole('STUDENT'))
                        @if (
                            $event->students()->byRoleEvent('HEAD')->where('user_id', Auth::user()->id)->exists() ||
            ...
            <div class="flex justify-between items-cente my-5">
                <div class="flex items-center">
                    <img src="{{ asset('/storage/' .$event->students()->byRoleEvent('HEAD')->first()->user_profile_img) }}"
                        alt="Avatar" class="w-8 h-8 rounded-full mr-2 object-cover">
                    <span
                        class="text-gray-800 font-semibold">{{ $event->students()->byRoleEvent('HEAD')->first()->user->user_firstname }}
                        {{ $event->students()->byRoleEvent('HEAD')->first()->user->user_lastname }}</span>
                </div>
                <div class="">
                    <h1>Participants {{ $event->event_participant }} / {{ $event->event_participant_limit }}</h1>
