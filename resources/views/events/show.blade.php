    ...
    <div class="">
        ...
        <div class=" m-32 mt-10">
            ...
            <div class="flex mt-20 justify-end">
                @if (Auth::check() && (Auth::user()->role === 'ADMIN' || Auth::user()->student->id === $event->student_id ))
                    @can('delete', $event)
                        <form action="{{ route('events.destroy', $event) }}" method="POST">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="bg-[#FF6666] p-1 pl-7 pr-7 mr-5 rounded transition-all hover:opacity-80">Delete</button>
                        </form>
                    @endcan
                @endif
